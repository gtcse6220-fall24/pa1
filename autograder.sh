...
if (( $(echo "$time1 > $max_time" | bc -l) )); then
        echo "Wrong answer: Time ($time1 seconds) exceeds $max_time seconds"
    else
        echo "Correct answer: Time ($time1 seconds) within $max_time seconds"
    fi
...
if (( $(echo "$speedup < $speedup_ref" | bc -l) )); then
    echo "Wrong answer: Speedup ($speedup) should be at least $speedup_ref for 16 processors"
else
    echo "Correct answer: Speedup ($speedup) is at least $speedup_ref for 16 processors"
fi
'
